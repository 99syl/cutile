...
int main(int ac, char** av)
{
    ...
    *val = 55;
    cutile_test_assert_m(*hash_table_get(&tbl, "1", 1) == 55);

    u8* val2 = hash_table_emplace(&tbl, "2", 1);
    *val2 = 128;
    cutile_test_assert_m(*hash_table_get(&tbl, "2", 1) == 128);

    u8* val5 = hash_table_emplace(&tbl, "5", 1);
    *val5 = 3;
    cutile_test_assert_m(*hash_table_get(&tbl, "5", 1) == 3);
    
    u8* val11 = hash_table_emplace(&tbl, "11", 2);
    *val11 = 12;
    cutile_test_assert_m(*hash_table_get(&tbl, "11", 2) == 12);

    cutile_test_assert_m(tbl.count == 4);

    hash_table_remove(&tbl, "1", 1);
    cutile_test_assert_m(hash_table_get(&tbl, "1", 1) == nullptr);
